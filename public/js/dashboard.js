...
let EDIT_HABIT_ID = null;

async function openEditHabitModal(habitId) {
  EDIT_HABIT_ID = habitId;
  const resp = await apiRequest(`/api/habits/${habitId}`);
  const h = resp.habit || resp;
  ...
  document.getElementById("edit-habit-startDate").value = (h.startDate || "").toString().slice(0, 10);

  const customDaysWrap = document.getElementById("edit-custom-days-container");
  const isCustom = (h.frequency === "custom");
  customDaysWrap.classList.toggle("d-none", !isCustom);

  document.querySelectorAll(".edit-day-checkbox").forEach(cb => cb.checked = false);
  const selected = h.schedule?.daysOfWeek || [];
  selected.forEach(d => {
    const el = document.querySelector(`.edit-day-checkbox[value="${d}"]`);
    if (el) el.checked = true;
  });

  const r0 = Array.isArray(h.reminders) && h.reminders.length ? h.reminders[0] : null;

  document.getElementById("edit-reminder-time").value = r0?.time || "";
  document.getElementById("edit-reminder-enabled").checked = r0 ? (r0.enabled !== false) : true;
  document.getElementById("edit-reminder-note").value = r0?.note || "";

  document.querySelectorAll(".edit-reminder-day").forEach(cb => cb.checked = false);
  (r0?.daysOfWeek || []).forEach(d => {
    const el = document.querySelector(`.edit-reminder-day[value="${d}"]`);
    if (el) el.checked = true;
  });
...
function collectEditReminderData() {
  const time = document.getElementById("edit-reminder-time")?.value;
  if (!time) return null;

  const days = Array.from(document.querySelectorAll(".edit-reminder-day:checked"))
    .map(cb => Number(cb.value));

  return {
    time,
    daysOfWeek: days,
    enabled: document.getElementById("edit-reminder-enabled").checked,
    note: document.getElementById("edit-reminder-note").value || ""
  };
}

async function saveEditedHabit() {
  if (!EDIT_HABIT_ID) return;
  ...
  const payload = {
    ...
    goal: {
      type: "count",
      target: parseInt(document.getElementById("edit-habit-target").value || "1", 10),
      unit: document.getElementById("edit-habit-unit").value.trim(),
    },
  };

  await apiRequest(`/api/habits/${EDIT_HABIT_ID}`, {
    method: "PATCH",
    body: payload,
  });

  const reminder = collectEditReminderData();

  await apiRequest(`/api/reminders/habits/${EDIT_HABIT_ID}/reminders`, {
    method: "PUT",
